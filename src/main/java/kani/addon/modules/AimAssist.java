...
public class AimAssist extends Module {
    ...
    public AimAssist() {
        super(KaniAddon.CATEGORY, "aim-assist", "Automatically aims at entities.");
    }
    ...
    private boolean inFov(Entity entity) {
        ...
        float[] angles = PlayerUtils.calculateAngle(entity.getBoundingBox().getCenter());
        double xDist = MathHelper.angleBetween(angles[0], mc.player.getYaw());
        double yDist = MathHelper.angleBetween(angles[1], mc.player.getPitch());
        double angleDistance = Math.hypot(xDist, yDist);
        return angleDistance <= fov.get();
    }
}
